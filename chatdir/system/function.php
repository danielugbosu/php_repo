...
$iconrank = '';
    if($post['user_rank'] > 1){
        $iconrank = ' ' . rankIcon($post, 2);
    }
    if(notMe($post['user_id'])){
        $add_av = 'onclick="openAvMenu(this, \'' . $post['user_name'] . '\', \'' . $post['user_id'] . '\');"';
    }
    return  '<li id="log' . $post['post_id'] . '" data="' . $post['post_id'] . '" class="ch_logs ' . $post['type'] . '">
                <div class="my_avatar chat_avatar_wrap" ' . $add_av . '>
                    <img class="avsex ' . sex($post['user_sex']) . ' avatar_userlist ' . ownAvatar($post['user_id']) . '" src="' . myavatar($post['user_tumb']) . '"/>
                </div>
			
                <div class="my_text">
				
                    <p><span class="adminss_icon">' . $iconrank . '</span><span class="username ' . $post['user_color'] . '">' . $post['user_name'] . '</span></p>
	...
	switch($status){
		...
		case 5:
			return curStatus($lang['eating'], 'cutlery', 'eating');
					case 7:
			return curStatus($lang['drankje'], 'glass', 'fa glass fa-glass');
					case 8:
			return curStatus($lang['Diamant'], 'diamond fa-spin', 'fa dijamant diamond fa-spin');
					case 9:
			return curStatus($lang['Bliksem'], 'fa-bolt', 'fa munja fa-bolt');
					case 10:
			return curStatus($lang['Smiley'], 'fa-smile-o', 'fa sretno-o fa-smile-o');
					case 11:
			return curStatus($lang['coffee'], 'fa-coffee', 'fa kavica fa-coffee');
					case 12:
			return curStatus($lang['Beer'], 'fa-beer', 'fa beer_time fa-beer');
					case 13:
			return curStatus($lang['Bed'], 'fa-bed', 'fa krevet fa-bed');
					case 14:
			return curStatus($lang['Hashtag'], 'fa-hashtag', 'fa hashtag fa-hashtag');
					case 16:
			return curStatus($lang['battery'], 'fa-battery-full', 'fa battery fa-battery-full');
			        case 17:
			return curStatus($lang['battery_empty'], 'fa-battery-empty', 'fa battery_empty fa-battery-empty');
					 case 18:
			return curStatus($lang['thermometer-full'], 'fa-thermometer-full', 'fa thermometer-full fa-thermometer-full');
					case 19:
			return curStatus($lang['sad'], 'fa-frown-o', 'fa frown-o fa-frown-o');
					case 20:
			return curStatus($lang['bug'], 'fa-bug', 'fa bug fa-bug');
					case 21:
			return curStatus($lang['blij'], 'fa-smile-o', 'fa smile fa-smile-o');
					case 15:
			return curStatus($lang['music'], 'fa-music', 'fa glazba fa-music');
								case 22:
			return curStatus($lang['love'], 'fa-heart', 'fa heart fa-heart faa-pulse animated');
		default:
			return '';
	}
}
...
function listStatus($status, $id = ''){
	global $lang;
	if($id == 1){
	return statusMenu($lang['online'], 'check-circle', 'userid1');
	}
	switch($status){
		case 1:
		...
		case 5:
			return statusMenu($lang['eating'], 'cutlery', 'eating');
					case 7:
			return statusMenu($lang['drankje'], 'glass', 'glass');
					case 8:
			return statusMenu($lang['Diamant'], 'diamond fa-spin', 'fa dijamant diamond fa-spin');
					case 9:
			return statusMenu($lang['Bliksem'], 'fa-bolt', 'fa munja fa-bolt');
					case 10:
			return statusMenu($lang['Smiley'], 'fa-smile-o', 'fa sretno-o fa-smile-o');
					case 11:
			return statusMenu($lang['coffee'], 'fa-coffee', 'fa kavica fa-coffee');
					case 12:
			return statusMenu($lang['Beer'], 'fa-beer', 'fa beer_time fa-beer');
					case 13:
			return statusMenu($lang['Bed'], 'fa-bed', 'fa krevet fa-bed');
					case 14:
			return statusMenu($lang['Hashtag'], 'fa-hashtag', 'fa hashtag fa-hashtag');
					case 15:
			return statusMenu($lang['music'], 'fa-music', 'fa glazba fa-music');
					case 16:
			return statusMenu($lang['battery'], 'fa-battery-full', 'fa battery fa-battery-full');
					case 17:
			return statusMenu($lang['battery_empty'], 'fa-battery-empty', 'fa battery_empty fa-battery-empty');
					case 18:
			return statusMenu($lang['thermometer-full'], 'fa-thermometer-full', 'fa thermometer-full fa-thermometer-full');
					case 19:
			return statusMenu($lang['sad'], 'fa-frown-o', 'fa frown-o fa-frown-o');
					case 20:
			return statusMenu($lang['bug'], 'fa-bug', 'fa bug fa-bug');
					case 21:
			return statusMenu($lang['blij'], 'fa-smile-o', 'fa smile fa-smile-o');
								case 22:
			return statusMenu($lang['love'], 'fa-heart', 'fa heart fa-heart faa-pulse animated');
		            case 6:
...
function listAllStatus(){
	...
	$list .= statusBox(5, listStatus(5));
		if(boomAllow(2)){
		$list .= statusBox(7, listStatus(7));
		$list .= statusBox(8, listStatus(8));
		$list .= statusBox(9, listStatus(9));
		$list .= statusBox(10, listStatus(10));
		$list .= statusBox(11, listStatus(11));
		$list .= statusBox(12, listStatus(12));
		$list .= statusBox(13, listStatus(13));
		$list .= statusBox(14, listStatus(14));
		$list .= statusBox(15, listStatus(15));
		$list .= statusBox(16, listStatus(16));
		$list .= statusBox(17, listStatus(17));
		$list .= statusBox(18, listStatus(18));
		$list .= statusBox(19, listStatus(19));
		$list .= statusBox(20, listStatus(20));
		$list .= statusBox(21, listStatus(21));
				$list .= statusBox(22, listStatus(22));
			}
	if(boomAllow(9)){
...
function rankIcon($list, $type = 1){
	...
	if(isBot($list['user_bot'])){
		return curRanking($lang['user_bot'], 'ico_bot', 'bot');
	}
	else if($list['user_role'] > 0 && $list['user_rank'] < 8 && $type == 2){
		switch($list['user_role']){
			case 5:
				return curRanking($lang['r_admin'], 'ico_radmin', 'cog');
			case 4:
		...
		switch($list['user_rank']){
			case 0:
				return curRanking($lang['guest'], 'ico_guest', '');
			case 1:
				return curRanking($lang['user'], 'ico_user', '');
			case 2:
				return curRanking($lang['vip'], 'ico_vip', 'crown');
			case 3:
				return curRanking($lang['jarig'], 'ico_jarig', 'birthday');
			case 4:
				return curRanking($lang['gold_member'], 'ico_goldmember', 'bookmark');
			case 8:
				return curRanking($lang['mod'], 'ico_mod', 'shield');
			case 9:
				return curRanking($lang['admin'], 'ico_admin', 'police');
			case 10:
				return curRanking($lang['owner'], 'ico_owner', 'police');
			case 11:
				return curRanking($lang['owner'], 'ico_owner', 'police');
			default:
				return curRanking($lang['user'], 'ico_user', '');
...
function listRank($current, $req = 0){
	...
	$rank .= '<option value="3" ' . selCurrent($current, 3) . '>' . $lang['jarig'] . '</option>';
	$rank .= '<option value="4" ' . selCurrent($current, 4) . '>' . $lang['gold_member'] . '</option>';
