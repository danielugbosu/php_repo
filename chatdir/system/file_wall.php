...
if (isset($_FILES["file"])){
	...
	if (isImage($extension)){
		...
		if ($imginfo !== false) {
			...
			move_uploaded_file(preg_replace('/\s+/', '', $_FILES["file"]["tmp_name"]), "../upload/wall/" . $file_name);
			$myimage = $data['domain'] . "/upload/wall/" . $file_name;
			$file_encrypt = encrypt($file_name);
			$file_type = 'image';
			$mysqli->query("INSERT INTO boom_upload (file_name, file_key, date_sent, file_user, file_zone, file_type, file_complete) VALUES ('$file_name', '$file_encrypt', '" . time() . "', '{$data['user_id']}', 'wall', 'image', 0)");
