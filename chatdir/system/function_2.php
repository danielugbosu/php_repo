...
function getLogo($type){
    global $mysqli, $data;
    $logo = $data['domain'] . '/default_images/' . $type . '.png';
    if(boomLogged()){
        if(boomAllow($data['allow_theme']) && $data['user_theme'] != 'system'){
            if(file_exists(__DIR__ . '/../css/themes/' . $data['user_theme'] . '/images/' . $type  . '.png')){
                $logo = $data['domain'] . '/css/themes/' . $data['user_theme'] . '/images/' . $type . '.png';
            }
        }
    }
    else {
        if(file_exists(__DIR__ . '/../css/themes/' . $data['default_theme'] . '/images/' . $type . '.png')){
            $logo = $data['domain'] . '/css/themes/' . $data['default_theme'] . '/images/' . $type . '.png';
        }
    }
    return $logo;
}
...
SELECT boom_chat.*, boom_users.user_name, boom_users.user_color, boom_users.bccolor, boom_users.user_tumb, boom_users.user_rank, boom_users.user_role, boom_users.user_bot, boom_users.user_sex
	FROM boom_chat
...
function colorChoice($sel, $type, $min = 1, $max = 48){
	$show_c = '';
	$c = '';
	switch($type){
		case 1:
...
function colorChoicetext($sel, $type, $min = 1, $max = 32){
	$show_c = '';
	$c = '';
	switch($type){
		case 1:
...
function colorChoiceadmin($sel, $type, $min = 50, $max = 53){
	$show_c = '';
	$c = '';
	switch($type){
		case 1:
...
function colorChoiceadminnames($sel, $type, $min = 60, $max = 70){
	$show_c = '';
	$c = '';
	switch($type){
		case 1:
...
function resetColors($id){
	...
	$mysqli->query("UPDATE boom_users SET user_color = '' WHERE user_id = '$id'");
	$mysqli->query("UPDATE boom_users SET user_mood = '' WHERE user_id = '$id'");
	$mysqli->query("UPDATE boom_users SET user_status = '' WHERE user_id = '$id'");
}
